...
class Leveler:
    ...
    async def profile(self,ctx, *, user : discord.Member=None):
        ...
        if "text_only" in self.settings and server.id in self.settings["text_only"]:
            ...
            await self.bot.send_file(channel, 'data/leveler/temp/{}_profile.png'.format(user.id), content='{}님의 프로필 입니다!'.format(self._is_mention(user)))
    ...
    async def rank(self,ctx,user : discord.Member=None):
        ...
        if "text_only" in self.settings and server.id in self.settings["text_only"]:
            ...
            await self.bot.send_typing(channel)
            await self.bot.send_file(channel, 'data/leveler/temp/{}_rank.png'.format(user.id), content='{}님의 정보입니다!'.format(self._is_mention(user)))
            db.users.update_one({'user_id':user.id}, {'$set':{
    ...
    async def info(self, ctx, *, info):
        ...
        if server.id in self.settings["disabled_servers"]:
            await self.bot.say("Leveler 명령어가 해지된 서버입니다.")
            return
    ...
    async def rankbg(self, ctx, *, image_name:str):
        ...
        if image_name in self.backgrounds["등수"].keys():
            if await self._process_purchase(ctx):
                db.users.update_one({'user_id':user.id}, {'$set':{"rank_background": self.backgrounds["등수"][image_name]}})
    ...
    async def textonly(self, ctx, all:str=None):
        ...
        if all != None:
            ...
            if server.id in self.settings["text_only"]:
                ...
                await self.bot.say("**`{}`에서는 모든 leveler명령어는 사진이 아닌 글로 나타납니다.".format(server.name))
            else:
                self.settings["text_only"].append(server.id)
                await self.bot.say("**`{}`에서는 모든 leveler명령어는 글이 아닌 사진으로 나타납니다.**".format(server.name))
        fileIO('data/leveler/settings.json', "save", self.settings)
    ...
    async def delete(self, ctx, *, name:str):
        ...
        if name in serverbadges['badges'].keys():
            ...
            await self.bot.say("**`{}`뱃지는 삭제 되었습니다.**".format(name))
        else:
            await self.bot.say("**그 뱃지는 찾지 못하였습니다.**")
    ...
    async def take(self, ctx, user : discord.Member, name: str):
        ...
        if name not in badges:
            await self.bot.say("**그 뱃지는 이 서버에 있지 않습니다!**")
        elif badge_name not in userinfo["badges"]:
            await self.bot.say("**{}는 그 뱃지를 가지고 있지 않습니다!**".format(self._is_mention(user)))
        else:
            if userinfo['badges'][badge_name]['price'] == -1:
                del userinfo["badges"][badge_name]
                db.users.update_one({'user_id':user.id}, {'$set':{"badges": userinfo["badges"]}})
                await self.bot.say("**{}는 `{}`뱃지를 뺏어갔습니다! :upside_down:**".format(self._is_mention(org_user), name, self._is_mention(user)))
            else:
                await self.bot.say("**살 수 있는 뱃지는 못 뺏어갑니다!!**")
    ...
    async def draw_profile(self, user, server):
        ...
        draw.text((self._center(0, 140, "랭킹", label_fnt), label_align), "랭킹",  font=label_fnt, fill=info_text_color) # Rank
        draw.text((self._center(0, 340, "레벨", label_fnt), label_align), "레벨",  font=label_fnt, fill=info_text_color) # Exp
        draw.text((self._center(200, 340, "돈", label_fnt), label_align), "돈",  font=label_fnt, fill=info_text_color) # Credits
    ...
    async def _handle_levelup(self, user, userinfo, server, channel):
        ...
        if server.id in self.settings["lvl_msg"]: # if lvl msg is enabled
            ...
            if "text_only" in self.settings and server.id in self.settings["text_only"]:
                await self.bot.send_typing(channel)
                em = discord.Embed(description='**{}님이 레벨업 하였습니다!{}! (레벨 {})**'.format(name, server_identifier, new_level), colour=user.colour)
                await self.bot.send_message(channel, '', embed = em)
            else:
                await self.draw_levelup(user, server)
                await self.bot.send_typing(channel)
                await self.bot.send_file(channel, 'data/leveler/temp/{}_level.png'.format(user.id), content='**{}님이 레벨업 하였습니다{}!**'.format(name, server_identifier))
