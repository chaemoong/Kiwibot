...
ACTIONS_REPR = {
    "BAN"     : ("벤", "\N{HAMMER}"),
    "KICK"    : ("킥", "\N{WOMANS BOOTS}"),
    "CMUTE"   : ("뮤트", "\N{SPEAKER WITH CANCELLATION STROKE}"),
    "SMUTE"   : ("뮤트", "\N{SPEAKER WITH CANCELLATION STROKE}"),
    "SOFTBAN" : ("부드러운 벤", "\N{DASH SYMBOL} \N{HAMMER}"),
    "HACKBAN" : ("핵벤", "\N{BUST IN SILHOUETTE} \N{HAMMER}"),
    "UNBAN"   : ("언벤", "\N{DOVE OF PEACE}")
}
...
class Mod:
    ...
    async def hackban(self, ctx, user_id: int, *, reason: str = None):
        ...
        if is_banned:
            await self.bot.say("그 유저는 이미 벤을 당하였습니다!")
            return
        ...
        try:
            ...
            await self.new_case(server,
                                ...
                                user=user,
                                reason=reason)
            await self.bot.say("완료. 사이다 한병 어떠세요"
                               "?")
    ...
    async def channel_mute(self, ctx, user : discord.Member, *, 사유: str = None):
        ...
        try:
            ...
            await self.new_case(server,
                                ...
                                user=user,
                                reason=reason)
            await self.bot.say("그 유저는 이 채널에서 발언권이 박탈 되었습니다!")
    ...
    async def channel_unmute(self, ctx, user : discord.Member):
        ...
        try:
            ...
            dataIO.save_json("data/mod/perms_cache.json", self._perms_cache)
            await self.bot.say("그 유저는 이 채널에서 발언권이 생성 되었습니다.")
    ...
    async def unban(self, ctx, userid):
        ...
        await self.new_case(ctx.message.server, user=userobj, action="UNBAN")
        await self.bot.say("완료되었습니다. 당신은 한명의 유저를 살렸습니다!")
