    ...
    async def _steam(self, context, *game: str):
        ...
        if match:
            ...
            if info:
                ...
                em.add_field(name='**가격**', value=info['price'])
                em.add_field(name='**장르**', value=', '.join([genre['description'] for genre in info['genres']]))
                em.add_field(name='\a', value='\a')
                em.add_field(name='**만든 날짜**', value=info['release_date'])
                em.add_field(name='**개발자**', value=', '.join([developer for developer in info['developers']]))
                em.add_field(name='\a', value=info['about_the_game'], inline=False)
                em.set_footer(text='Grand Kiwi Bot의 steam cog는 Steam에 의해 돌아갑니다!', icon_url='http://store.akamai.steamstatic.com/public/shared/images/header/globalheader_logo.png')
